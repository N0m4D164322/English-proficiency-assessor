...
#include <unistd.h>
#include <math.h>
#include <ctype.h> // For sleep function
#include "red.h"
...
// Function to search for the executable and open it in a new terminal window
void oe(const char *executableName) {
    char command[COMMAND_SIZE];
    char path[COMMAND_SIZE];

    // Use the 'where' command to find the executable
    snprintf(command, sizeof(command), "where %s", executableName);

    // Create a file pointer to capture the output of the command
    ...
    // Read the output and get the path
    if (fgets(path, sizeof(path), fp) != NULL) {
        // Remove newline character from the path
        path[strcspn(path, "\n")] = 0;

        // Prepare the command to open the executable in a new terminal window
        snprintf(command, sizeof(command), "start cmd /k \"%s\"", path);

        // Execute the command
        int result = system(command);

        if (result == -1) {
            printf("Failed to open the executable in a new window.\n");
        } else {
            printf("Opened %s in a new terminal window.\n", path);
        }
    } else {
        printf("%s not found.\n", executableName);
    }
...
void initializeTopics(Topic *topics) {
    ...
    clearScreen();
    topics[0] = (Topic) {
        .explanation = "Sir, jei topic e issue dekhabe, oi topic ta amake diyen, ami pdf pathay dibone\n"
...
void interactiveTopic(Topic topic) {
    ...
    printf("\nPress 'Enter' to see the problem set...\n");
...
void displayBanner() {
    //printf("  "); // Set text color to green
    clearScreen();
    printf("========================================\n");
    printf("        ||         STOA         ||      \n");
    printf("========================================\n");
    printf("This Program was created by Atif Ahnaf Chowdhury. copyright-2024\n");
...
void displayMenu() {
    ...
    printf("  [A] Check your level of writing in IELTS band\n");
    printf("  [B] Grammar practice\n");
    printf("  [Q] Quit\n");
    printf("\033[0m"); // Reset text color
}
...
void executeOption(char choice) {
    ...
    switch (choice) {
        case 'A':
        case 'a':
            // Clear input buffer before calling readability
            while (getchar() != '\n'); // Clear invalid input
            readability(); // Call readability to prompt for input
            sleep(2); // Simulate delay after input collection
            printf("\033[0m");
            break;
        case 'B':
        case 'b':
            printf("Loading Grammar Practice...\n");
            sleep(2); // Simulate task execution with a delay
            printf("\033[0m");
            Topic topics[10]; // Adjust size based on the number of topics
            initializeTopics(topics);

            // Run through each topic
            for (int l = 0; l < 10; l++) { // Loop through all topics
                interactiveTopic(topics[l]);
            }
            break;
        case 'Q':
        case 'q':
...
int main() {
    ...
    while (1) {
        ...
        if (scanf(" %c", &choice) != 1) { // Check if the input is successful
            printf("Invalid input. Please try again.\n");
            while(getchar() != '\n'); // Clear invalid input
            continue; // Restart the loop
        }
        executeOption(choice);
        printf("\n"); // Optional: adds a blank line after each choice
    }
    return 0; // This should be outside the loop
}
